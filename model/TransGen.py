...
def updown(x, H, W):
    ...
    x = x.permute(0, 2, 1)
    x = x.view(-1, C, H, W)
    x = nn.functional.interpolate(x, scale_factor=4, mode='bicubic',align_corners=True)
    x = nn.AvgPool2d(4)(x)
    B, C, H, W = x.size()
    x = x.view(-1, C, H*W)
    x = x.permute(0,2,1)
...
class Decoder(nn.Module):
    def __init__(self, img_size=128, patch_size=16, in_chans=3,  embed_dim=384, depth=5,
        ...
        mlp_ratio = 4.
        depth = [5,5,4,4,4,4]
        act_layer = "gelu" #leakyrelu
        self.l2_size = 0
        
        if self.l2_size == 0:
            self.l1 = nn.Linear(control_num, (self.bottom_width ** 2) * self.embed_dim)
        elif self.l2_size > 1000:
            self.l1 = nn.Linear(control_num, (self.bottom_width ** 2) * self.l2_size//16)
            self.l2 = nn.Sequential(
                        nn.Linear(self.l2_size//16, self.l2_size),
                        nn.Linear(self.l2_size, self.embed_dim)
                      )
        else:
            self.l1 = nn.Linear(control_num, (self.bottom_width ** 2) * self.l2_size)
            self.l2 = nn.Linear(self.l2_size, self.embed_dim)
        self.embedding_transform = nn.Linear(control_num, (self.bottom_width ** 2) * self.embed_dim)
...
#                         )
        ...
        self.deconv = nn.Sequential(
            nn.Conv2d(self.embed_dim//16, 3, 3, 1, 1)
        )
